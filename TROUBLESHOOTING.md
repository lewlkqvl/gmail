...
**已解决**: 最新版本使用了**双重保护机制**来防止此错误：

1. **主动拦截（最重要）**: 使用 `page.on('dialog')` 事件监听器主动捕获并自动关闭所有对话框
2. **函数覆盖（双重保险）**: 使用 `evaluateOnNewDocument()` 覆盖 `prompt()`、`alert()`、`confirm()` 函数
3. **Chrome 参数**: 添加 `--disable-popup-blocking` 等参数进一步防止弹窗

**技术细节**:
```javascript
// 关键代码片段（已在最新版本中实现）
page.on('dialog', async dialog => {
  console.log(`检测到对话框: ${dialog.type()} - 已自动关闭`);
  await dialog.dismiss();
});
```

**如果仍然出现**:
- 确保已更新到最新代码（需要拉取最新的提交）
- 检查浏览器控制台输出，应该会看到 `[Dialog 拦截]` 相关日志
- 查看 `/tmp/autologin_*.png` 截图确认页面状态
...
**说明**: Google 可能在登录过程中要求完成人机验证（reCAPTCHA），这是正常的安全措施。

**自动处理**: 最新版本已支持**智能等待人机验证**：

1. **自动检测**: 系统会自动检测页面中的验证码元素
2. **暂停流程**: 检测到验证后暂停自动化流程
3. **等待完成**: 最多等待 5 分钟供用户手动完成验证
4. **自动继续**: 验证完成后自动恢复流程

**使用说明**:
- 当看到提示 `🤖 检测到人机验证` 时，在浏览器窗口中手动完成验证
- 通常是勾选"我不是机器人"或选择图片
- 完成后系统会自动检测并继续（约 2 秒内）
- 每 30 秒会提示剩余等待时间
...
**常见验证类型**:
- ✅ Google reCAPTCHA v2（勾选框）
- ✅ Google reCAPTCHA v3（无感知）
- ✅ 图片选择验证
- ✅ 滑块验证
...
### v1.3.0 (最新)
- ✅ **新功能**: 智能检测并等待人机验证（reCAPTCHA）
- ✅ 自动暂停流程，等待用户手动完成验证（最多 5 分钟）
- ✅ 支持多种验证码类型（reCAPTCHA v2/v3、图片选择、滑块等）
- ✅ 验证完成后自动恢复自动化流程
- ✅ 每 30 秒提示剩余等待时间

### v1.2.0
- ✅ **关键修复**: 使用 `page.on('dialog')` 主动拦截对话框，彻底解决 `prompt()` 错误
- ✅ 添加双重保护机制（事件监听 + 函数覆盖）
- ✅ 增强 Chrome 启动参数（`--disable-popup-blocking` 等）
- ✅ 改进对话框检测和日志输出
- ✅ 更详细的调试信息
